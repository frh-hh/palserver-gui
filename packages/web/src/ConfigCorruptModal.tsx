...
export function ConfigCorruptModal({
  ...
  const label = file === "world" ? "世界設定檔(PalWorldSettings.ini)" : "效能設定檔(Engine.ini)";
  ...
  const regenerate = async () => {
    ...
    try {
      ...
      alert(
        res.backedUp
          ? `已生成新的設定檔。原損壞檔案已備份為 ${res.path}.corrupt-*.bak`
          : "已生成新的設定檔。",
      );
    } catch (err) {
    ...
    <Overlay onClose={() => {}}>
      <div className={`${card} flex w-[460px] max-w-full flex-col gap-3`} onClick={(e) => e.stopPropagation()}>
        <h2 className="inline-flex items-center gap-2 text-lg font-extrabold text-berry">
          <FiAlertTriangle className="size-5" /> 設定檔已損壞
        </h2>
        <p className="text-[13px] text-ink-muted">
          {label}無法正確解析,伺服器可能會忽略設定或開出一個全新的世界。
        </p>
        {health.reason && (
        ...
        <p className="text-[13px] text-ink-muted">
          可以生成一份新的設定檔
          {file === "world" ? "(依 GUI 目前儲存的世界設定)" : ""}。原本的損壞檔案會先備份起來,不會直接刪除。
        </p>
